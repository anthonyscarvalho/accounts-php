...
class statements
{
    public function email()
    {
        ...
        {
            global $db;
            $_pdfData = performAction( 'statements', 'setupPDF', [$_POST['clients'], $_POST['startDate'], $_POST['endDate']] );
            ...
            $_contactSql = "SELECT * FROM contacts WHERE clients='" . $_POST['clients'] . "' AND canceled='false';";
            ...
            if ( $_totContacts > 0 )
            {
                ...
                foreach ( $_contacts['data'] as $_contact )
                {
                    ...
                    $emailer->setAttachment( 'statements', $_invoicePDF, $_pdfData['pdfName']);
    ...
    public function printState( $_clients = null, $_start = null, $_end = null )
    {

        if ( !empty( $_clients ) )
        {
            $_pdfData = performAction( 'statements', 'setupPDF', [$_clients, $_start, $_end] );
    ...
    public function setupPDF( $_clients = null, $_start = null, $_end = null )
    {
        global $db;
        $_POST['view_type'] = 'view';
        $_POST['clients'] = $_clients;

        if ( !empty( $_start ) )
        {
            $_POST['startDate'] = $_start;
        }
        else
        {
            $_invoiceDate = $db->select( "SELECT creation_date FROM invoices WHERE clients='" . $_clients . "' AND canceled='false' ORDER BY creation_date ASC LIMIT 0,1", 'true' );
            $_transDate = $db->select( "SELECT date FROM transactions WHERE clients='" . $_clients . "' ORDER BY date ASC LIMIT 0,1", 'true' );

            if ( $_invoiceDate['data']['creation_date'] > $_transDate['data']['date'] )
            {
                $_POST['startDate'] = $_invoiceDate['data']['creatoin_date'];
            }
            else
            {
                $_POST['startDate'] = $_transDate['data']['date'];
            }
...
// get the opening balance
                if ( $_POST['startDate'] != '' )
                {
                    ...
                    $_res = $db->select( "SELECT SUM(invoice_total) AS totDebit FROM invoices WHERE clients=" . $_POST['clients'] . " AND due_date<'" . $_POST['startDate'] . "' AND canceled='false';", 'true' );
