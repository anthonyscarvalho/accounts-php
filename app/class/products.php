                    ...
                    foreach ($_companies as $_company) {
                        ...
                        if ($_totProd > 0) {
                            ...
                            foreach ($res['data'] as $prod) {
                                ...
                                $_prodMonthDiff = diff_months($originalDate, $prod['date']);
                                $_prodYearDiff = diff_years($originalDate, $prod['date']);

                                //Difference between last invoice date and posted date
                                $_invoiceMonthDiff = diff_months($originalDate, $prod['lastInvoice'] ?? $prod['date']);
                                $_invoiceYearDiff = diff_years($originalDate, $prod['lastInvoice'] ?? $prod['date']);
