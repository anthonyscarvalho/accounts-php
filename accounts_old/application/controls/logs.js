app.controller( 'viewLogs', function( $scope, $http, $rootScope, AuthManager, $uibModal, AuthManager ) {
    ...
    $scope.load = function() {
        AuthManager.get( '/logs/view', { view_type: 'view' } ).then( function( results ) {
    ...
    $scope.preview = function( $id ) {
        var uibModalInstance = $uibModal.open( {
            animation: false,
            templateUrl: 'application/views/logs/preview.php',
...
app.controller( 'searchLogs', function( $scope, $http, $routeParams, $rootScope, $uibModal, AuthManager ) {
    ...
    $scope.load = function() {
        AuthManager.get( '/logs/view', { view_type: 'search', id: $scope.parent } ).then( function( results ) {
            $scope.results = results.data;
...
app.controller( 'previewClientLog', function( $scope, $http, AuthManager, $uibModalInstance, id ) {
    ...
    AuthManager.get( '/logs/view', { view_type: 'edit', id: id } ).then( function( results ) {
        $scope.results = results.data;
