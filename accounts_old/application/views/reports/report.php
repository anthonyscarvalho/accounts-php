        ...
        <div class="well well-sm">
            <div class="form-group">
                ...
                <div class="btn-group" role="group" data-toggle="buttons">
                    <label class="btn btn-default" ng-model="reportType" uib-btn-radio="'income'">Income</label>
                    <label class="btn btn-default" ng-model="reportType" uib-btn-radio="'expense'">Expense</label>
                </div>
            </div>
            <div class="form-group">
                <label>Year</label>
    ...
    <div class="col-md-8">
        <div ng-if="reportType=='income'">
            <table class="table table-condensed table-hover table-striped" id="overview">
                ...
                <tbody ng-repeat="res in results" ng-init="val = updateInc( res.data )">
                    <tr>
                        <td>{{res.date}}</td>
                        <td colspan="5"></td>
                    </tr>
        ...
        <div ng-if="reportType=='expense'">
            <table class="table table-condensed table-hover table-striped click" id="incomeOverview">
                ...
                <tbody ng-repeat="res in results" ng-init="val = updateExp( res.data )">
                    <tr>
                        <td>{{res.date}}</td>
                        <td colspan="5"></td>
                    </tr>
                    <tr ng-repeat="ress in res.data">
                        <td></td>
                        <td>{{ress.date}}</td>
                        <td>{{ress.companyName}}</td>
                        <td>{{ress.categoryName}}</td>
                        <td>{{ress.description}}</td>
                        <td class="text-right">{{ress.amount}}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="text-right">Total</td>
                        <td>{{val| currency:'R '}}</td>
