...
{
    $d1 = getdate(date(strtotime($_date1)));
    $d2 = getdate(date(strtotime($_date2)));
...
function sendEmail($recipient, $subject, $body, $name, $attachment, $cc = null)
{
...
//Set up the mail method
    ...
    {
        $transport = Swift_SmtpTransport::newInstance(emailServer, 587)->setUsername(emailAddress)->setPassword(emailPassword);
    }
...
//Create admin email to be sent
    ...
    $admin->setTo([$recipient['email'] => $recipient['name'] . " " . $recipient['surname']]);
    if ($cc == 'true')
    {
        $admin->setBcc([emailAddress => emailCompany]);
    }
    $admin->setBody($body, 'text/html');
    if ($attachment != "")
    {
...
//Send the email contact
    $result = $mailer->send($admin);

    $_result['status'] = $result;
    $_result['server_log'] = $logger->dump();
