...
class email_log
{
    ...
    {
        global $db;
        $_sql = "SELECT email_log.id, (users.name) AS userName, (contacts.name) AS contactName, email_log.subject, email_log.body, email_log.date, email_log.status, email_log.invoices "
            . "FROM contacts RIGHT JOIN (users RIGHT JOIN email_log ON users.id=email_log.users) ON contacts.id=email_log.contacts "
