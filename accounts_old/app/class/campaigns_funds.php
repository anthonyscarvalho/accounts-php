                    ...
                    if ($_POST['email'] == "true" && isset($_POST['clients']) && isset($_POST['credit']))
                    {
                        ...
                        if ($total_contacts > 0)
                        {
                            ...
                            foreach ($contacts['data'] as $contact)
                            {
                                ...
                                $res2 = sendEmail($contact, $subject, $body, '', '');

                                $_vars2['contacts'] = $contact['id'];
                                $_vars2['date'] = current_dateTime();
                                $_vars2['status'] = $res2['server_log'];
                                $_vars2['body'] = $body;
                                $_vars2['attachment'] = null;
                                performAction('manager', 'updateLog', ['ad_emails', $_vars2]);
                            }
                        }
                    }
                }
