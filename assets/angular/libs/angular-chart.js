...
Chart.Tooltip.positioners.myCustomPosition = function(unused, position) {
   return { x: position.x, y: 30 };
}
    Chart.defaults.global.tooltips.callbacks.label = function ( tooltipItem )
    {
        return addCommas( tooltipItem.yLabel );
    };
    // Chart.defaults.global.multiTooltipTemplate = '<%if (datasetLabel){%><%=datasetLabel%>: <%}%><%= addCommas(value) %>';
    ...
    function ChartJsProvider()
    {
        ...
        this.setOptions = function ( type, customOptions )
        {
            ...
            else
            {
                ...
                options[ type ] = angular.merge( options[ type ] ||
                {}, customOptions );
            }
    ...
    function ChartJsFactory( ChartJs, $timeout )
    {
        ...
        {
            return angular.extend(
            {}, ChartJs.getOptions( type ), scope.chartOptions );
        }
